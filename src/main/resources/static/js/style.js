...
function getCategory_List(e) {
    ...
    // 숙제
    // 카테고리 key 값은 그룹번호 + 카테고리ID로 만들어집니다
    // 클릭한 대상의 이 값을 인풋태그[name=prodCategory]에 value 값을 지정

    $(e.target).category_set();

    // var categoryKey = obj.groupId + obj.categoryId;
    // $("input[name='prodCategory']").val(categoryKey);

}
// 카테고리 세팅
$.fn.category_set = function() {
    var categoryId = this.data("set").categoryId; // 클릭한 대상의 categoryId
    var groupId = this.data("set").groupId; // 클릭한 대상의 groupId
    $("input[name='prodCategory']").val(groupId + categoryId ); //name이 prod_category인 곳에 추가
}
// 이전 카테고리 삭제 JS
